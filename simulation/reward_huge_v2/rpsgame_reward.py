...
#   VERSION : production engine, gen-2
...
def _run_and_extract(args):
    """Worker: run one simulation and return:
      ...
      - reward statistics (new): per-run win/defeat/quits counts and
...
def run_pilot(output_dir='./data/reward_pilot', n_workers=None,
    ...
    for init_mode, predict_mode in designs:
        ...
        for a1, a2 in pairs:
            ...
            t0 = time.time()
            agg, reward_stats = parallel_runs(
...
def run_grid(output_dir='./data/reward_grid', scale='small',
    ...
    for (a1, a2), m in product(pairs, windows):
        ...
        # ---- Resume support ----
        # Both durations and rewards files must exist for cache hit;
        # otherwise re-run the simulation. Old caches without rewards
        # will trigger re-run.
        agg = None
        reward_stats = None
        if durations_path.exists() and rewards_path.exists():
            try:
                ...
                agg = None; reward_stats = None
        elif durations_path.exists() and not rewards_path.exists():
            print(f"[{idx}/{total}] {cond_id}  (re-running: rewards data missing)")
            agg = None; reward_stats = None
    ...
    # Display key columns including reward summary
    cols_show = ['a1', 'a2', 'window']
    if 'win_rate_mean' in summary.columns:
        cols_show += ['win_rate_mean', 'cumR_per_step_mean', 'p_val_vs_chance']
...
def main():
    ...
    if args.cmd == 'pilot':
        run_pilot(args.output, n_workers=args.workers, full_design=args.full)
    elif args.cmd == 'grid':
