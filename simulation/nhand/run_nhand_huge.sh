...
echo "  Design:    rs (random init + sample predict)"
...
# Optional sanity-check pilot first (fast, ~1 min total) — verify rule works
echo "--- Sanity check: short pilot at each N (~1 min total) ---"
for N in "${N_VALUES[@]}"; do
    echo ""
    echo "[pilot N=$N] Running short verification..."
    python3 "$NHAND_PY" --workers "$WORKERS" pilot \
        --N "$N" \
        --output "./data/nhand_pilot_N${N}"
done
echo ""
echo "--- Pilot OK; proceeding to huge runs ---"
echo ""

# Main runs
for N in "${N_VALUES[@]}"; do
    OUTDIR="./data/nhand_grid_N${N}_rs"
    echo ""
    echo "=========================================="
