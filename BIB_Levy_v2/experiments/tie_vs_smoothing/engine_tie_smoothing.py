"""
[ABLATION COPY] Derived from simulation_tie_mode_ablation/files/rpsgame_reward_tie.py
with an added `jm_smoothing` flag (default True) that gates the conditional
Jelinek-Mercer smoothing (Eq. 5). Set jm_smoothing=False to disable it.
Used by build_tie_vs_smoothing.py to test whether tie->uniform observation
substitutes for the smoothing.

Reward-based BIB inference for Rock-Paper-Scissors
====================================================
[TIE-MODE ABLATION VARIANT -- extends rpsgame_reward.py from
 simulation_r4_fallback/files/ with a `tie_mode` parameter.]

Faithful translation of Ibuka & Sasai 2024 (ICA conference) Daihinmin
implementation to the 1v1 RPS setting.

Observation rule (Eq. 3 of main_PRE.tex), with new `tie_mode` switch:
  - win   -> d^t = my_hand               (my hand was good)
  - quits -> behavior controlled by tie_mode (see below)
  - defeat -> d^t = uniformly random     (my hand was bad,
              from {hands} - {my_hand}    look elsewhere)

tie_mode values:
  'skip'    (DEFAULT) -- no update on draw (Eq. 3 as published; same as
                         original rpsgame_reward.py; bit-identical when
                         the same seed is used and defeat_mode is also
                         left at its default).
  'uniform' -- d^t ~ Uniform({r,p,s}) i.e. uniformly random observation
               sampled over ALL hands including my_hand. This is the
               "case A" zero-mean noise injection.

defeat_mode values:
  'random_other' (DEFAULT) -- d^t ~ Uniform({hands} - {my_hand})
                              (Eq. 3 as published).
  'opponent'     -- d^t = opp_hand (the actual hand that beat me).
                    This is the "hybrid" scheme that uses the ground-
                    truth observation on defeat, bringing the reward-
                    based rule closer to the direct observation rule
                    of rpsgame_levy.py (Gunji-type) on losing rounds
                    while keeping the reward-based win and tie rules.
                    Together with tie_mode='uniform', this gives the
                    fully-symmetric scheme:
                       win    -> observe d_A  (self reinforcement)
                       tie    -> observe random
                       defeat -> observe d_B  (opponent's winning hand)

REVERSION POLICY: this file is fully self-contained and lives in its
own directory. Delete /simulation_tie_mode_ablation/ to revert; the
original simulation_r4_fallback/files/rpsgame_reward.py is untouched.
...
Posterior regularization (Ibuka & Sasai 2024 form, equivalent to
conditional Jelinek-Mercer interpolation):
  After each Bayesian update, if min P(h) < 0.002, apply
        P(h) <- 0.91 P(h) + 0.015
  and renormalize. This is mathematically equivalent to a
  *conditional* Jelinek-Mercer interpolation [Jelinek & Mercer 1980;
  Chen & Goodman 1996] toward the uniform reference, with strength
        α_JM(N_h) = 0.015 N_h / (0.91 + 0.015 N_h),
  i.e. α_JM ≈ 0.14 at N_h = 10. The conditional triggering (in
  contrast to always-on JM) is essential to the dynamics: an
  always-on JM with the same average strength produces qualitatively
  different statistics (verified — see Supplementary).

Likelihood smoothing on inverse step (Ibuka & Sasai 2024):
  If any entry of the replaced P(d|min_h) is below 1e-6, apply
        L <- 0.95 L + 0.05 / N_d
  (a small Laplace-style additive smoothing).

References:
  Ibuka, R. & Sasai, K. (2024). Inverse Bayesian Inference for
  Player Agents in Computer Daihinmin Game. IEEE ICA — original
  reward-based observation rule and conditional smoothing form.
  Gunji, Y.-P. et al. (2018). Inverse Bayesian inference in
  swarming behaviour of soldier crabs. Phil. Trans. R. Soc. A,
  376, 20170370 — argmin hypothesis replacement mechanism.
  Jelinek, F. & Mercer, R. L. (1980). Interpolated estimation of
  Markov source parameters from sparse data. In: Pattern
  Recognition in Practice, North-Holland, 381–397 — original
  Jelinek-Mercer interpolation method.
  Chen, S. F. & Goodman, J. (1996). An empirical study of smoothing
  techniques for language modeling. ACL, 310–318 — modern review
  of smoothing techniques in language modeling.
  Manning, C. D. & Schütze, H. (1999). Foundations of Statistical
  Natural Language Processing. MIT Press, §6.2 — textbook on
  smoothing.
...
class BayesReward:
    ...
    def __init__(self, h_num=10, d_num=3, d_type=None, h_length=50,
        ...
        # ABLATION FLAG: gate the conditional Jelinek-Mercer smoothing (Eq. 5).
        self.jm_smoothing = jm_smoothing
        self.h_num = h_num
        self.d_num = d_num
    ...
    def inference(self, data):
        ...
        # Conditional Jelinek-Mercer interpolation [Jelinek & Mercer
        # 1980; Chen & Goodman 1996], in the form of Ibuka & Sasai
        # 2024: when any P(h) drops below the trigger 0.002, pull
        # toward the uniform reference. Equivalent to JM with
        # α_JM(N_h) = 0.015 N_h / (0.91 + 0.015 N_h) ≈ 0.14 at N_h=10.
        # The conditional structure (not always-on) is essential to
        # the long-tailed argmax-persistence dynamics.
        if self.jm_smoothing and (self.h_prov < 0.002).any():
    ...
    def inverse(self):
        ...
        # Conditional additive smoothing (Ibuka & Sasai 2024 form):
        # if any entry is below 1e-6, mix in a small uniform component.
        # This is a Laplace-style additive smoothing [Laplace 1812;
        # Manning & Schütze 1999, §6.2.2] applied conditionally.
...
class AgentReward:
    ...
    def __init__(self, a_type, h_length=50, h_num=10,
                 ...
                 tie_mode='skip', defeat_mode='random_other', seed=None,
                 jm_smoothing=True):
        """
        a_type:
        ...
        tie_mode (NEW, ablation parameter):
          'skip'    -- on draw, no update (matches Eq. 3 published form;
                       bit-identical to original rpsgame_reward.py)
          'uniform' -- on draw, kansoku ~ Uniform({r,p,s}) (zero-mean
                       additive noise injection, case A)
        defeat_mode (NEW, ablation parameter):
          'random_other' -- on defeat, kansoku ~ Uniform({hands}-{my_hand})
                            (matches Eq. 3 published form)
          'opponent'     -- on defeat, kansoku = opp_hand (the hand that
                            actually beat me; ground-truth observation)
        """
        if tie_mode not in ('skip', 'uniform'):
            raise ValueError(f"tie_mode must be 'skip' or 'uniform', "
                             f"got {tie_mode!r}")
        if defeat_mode not in ('random_other', 'opponent'):
            raise ValueError(f"defeat_mode must be 'random_other' or "
                             f"'opponent', got {defeat_mode!r}")
        self.type = a_type
        self.tie_mode = tie_mode
        self.defeat_mode = defeat_mode
        self.rng = np.random.default_rng(seed)
        if a_type == 'random':
            self.bayes = None
        else:
    ...
    def update_from_outcome(self, my_hand, opp_hand):
        """Reward-based update with ablation switches.

        win   -> kansoku = my_hand  (always; published Eq. 3)
        quits -> behavior depends on self.tie_mode:
                 'skip'    : no update (default; matches Eq. 3)
                 'uniform' : kansoku ~ Uniform({r,p,s}) (case A)
        defeat -> behavior depends on self.defeat_mode:
                 'random_other' : kansoku ~ Uniform({hands}-{my_hand})
                                  (default; matches Eq. 3)
                 'opponent'     : kansoku = opp_hand
                                  (hybrid scheme: observe ground truth)

        NOTE on RNG consumption: when tie_mode='skip' AND
        defeat_mode='random_other', this function consumes the same
        random numbers as the original rpsgame_reward.py for any given
        seed, so behavior is bit-identical. Any other combination
        diverges after the first deviating event.
        """
        if self.bayes is None:
            return
        ...
        if result == 'quits':
            if self.tie_mode == 'skip':
                return  # original Eq. 3 behavior
            elif self.tie_mode == 'uniform':
                # Case A: zero-mean noise injection. Sample from ALL
                # hands including my_hand, so the expected likelihood
                # update is the marginal P(d|h) averaged over uniform d,
                # which equals 1/N_d for all h -- i.e. no systematic
                # bias, only stochastic perturbation.
                kansoku = self.rng.choice(HANDS)
            else:
                raise ValueError(f"Unexpected tie_mode={self.tie_mode!r}")
        elif result == 'win':
            kansoku = my_hand
        else:  # defeat
            if self.defeat_mode == 'random_other':
                others = [h for h in HANDS if h != my_hand]
                kansoku = self.rng.choice(others)
            elif self.defeat_mode == 'opponent':
                # Ground-truth observation: the hand that actually beat
                # us. For N=3 k=1 this is fully determined by my_hand
                # alone (and equals (my_hand + 1) mod N in cyclic
                # order), but using opp_hand keeps the code N-agnostic.
                kansoku = opp_hand
            else:
                raise ValueError(f"Unexpected defeat_mode={self.defeat_mode!r}")
...
def run_pair(a1, a2, n_steps,
    ...
    ag1 = AgentReward(a1, h_length, h_num, init_mode, predict_mode,
                      tie_mode=tie_mode, defeat_mode=defeat_mode, seed=seed1,
                      jm_smoothing=jm_smoothing)
    ag2 = AgentReward(a2, h_length, h_num, init_mode, predict_mode,
                      tie_mode=tie_mode, defeat_mode=defeat_mode, seed=seed2,
                      jm_smoothing=jm_smoothing)
...
def _run_and_extract(args):
    ...
    (a1, a2, n_steps, analysis_start,
     h_length, h_num, init_mode, predict_mode,
     tie_mode, defeat_mode, seed, jm_smoothing) = args
...
def parallel_runs(a1, a2, n_steps, n_runs, analysis_start,
                  ...
                  n_workers=None, seed_base=0, jm_smoothing=True):
    ...
    job_args = [
        (a1, a2, n_steps, analysis_start,
         h_length, h_num, init_mode, predict_mode,
         tie_mode, defeat_mode, seed_base + run, jm_smoothing)
...
def run_pilot(output_dir='./data/reward_pilot', n_workers=None,
              full_design=False, h_num=10, tie_mode='skip',
              defeat_mode='random_other'):
    ...
    print(f"  tie_mode={tie_mode!r}, defeat_mode={defeat_mode!r}")
    ...
    for init_mode, predict_mode in designs:
        design_tag = (f"init={init_mode}_pred={predict_mode}"
                      f"_tie={tie_mode}_def={defeat_mode}")
        print(f"\n--- {design_tag} ---")
...
def run_grid(output_dir='./data/reward_grid', scale='small',
             init_mode='random', predict_mode='sample',
             h_num=10, tie_mode='skip', defeat_mode='random_other',
             n_workers=None):
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    if n_workers is None:
    ...
    print(f"Reward-based BIB grid: scale={scale}, init={init_mode}, "
          f"predict={predict_mode}, h_num={h_num}, tie_mode={tie_mode!r}, "
          f"defeat_mode={defeat_mode!r}, workers={n_workers}")
    ...
    for (a1, a2), m in product(pairs, windows):
        idx += 1
        # Embed tie_mode + defeat_mode in cond_id so caches don't collide
        cond_id = (f"{a1}-{a2}_m{m}_tie{tie_mode}_def{defeat_mode}"
                   f"_{scale}")
        ...
        partial.to_csv(out / f'summary_{scale}_tie{tie_mode}_def{defeat_mode}_partial.csv',
                       index=False)

    summary = pd.DataFrame(rows)
    summary_path = out / f'summary_{scale}_tie{tie_mode}_def{defeat_mode}.csv'
...
def main():
    ...
    p_pilot.add_argument('--tie-mode', dest='tie_mode',
                         choices=['skip', 'uniform'], default='skip',
                         help='Tie handling: skip (default, Eq. 3) or '
                              'uniform (case A, zero-mean noise)')
    p_pilot.add_argument('--defeat-mode', dest='defeat_mode',
                         choices=['random_other', 'opponent'],
                         default='random_other',
                         help='Defeat handling: random_other (default, '
                              'Eq. 3) or opponent (hybrid, observe d_B)')
    ...
    p_grid.add_argument('--tie-mode', dest='tie_mode',
                        choices=['skip', 'uniform'], default='skip',
                        help='Tie handling: skip (default, Eq. 3) or '
                             'uniform (case A, zero-mean noise)')
    p_grid.add_argument('--defeat-mode', dest='defeat_mode',
                        choices=['random_other', 'opponent'],
                        default='random_other',
                        help='Defeat handling: random_other (default, '
                             'Eq. 3) or opponent (hybrid, observe d_B)')
